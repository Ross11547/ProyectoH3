...
def ajustar_tiempo_verde(num_autos):
    if num_autos < 5:
        return min_verde
    elif num_autos < 15:
        return tiempo_verde
...
duracion_actual = ajustar_tiempo_verde(num_autos=10)

def actualizar_semaforos(num_autos_vertical, num_autos_horizontal):
    ...
    if time.time() - tiempo_cambio >= duracion_actual:
        if semaforos["vertical"] == "verde":
            ...
            duracion_actual = ajustar_tiempo_verde(num_autos_horizontal)
        else:
            semaforos["vertical"] = "verde"
            semaforos["horizontal"] = "rojo"
            duracion_actual = ajustar_tiempo_verde(num_autos_vertical)
        tiempo_cambio = time.time()
...
while True:
    for evento in pygame.event.get():
        if evento.type == pygame.QUIT:
            pygame.quit()
            sys.exit()
    actualizar_semaforos(num_autos_vertical=12, num_autos_horizontal=5)
    dibujar_escena()
    pygame.display.flip()
    reloj.tick(30)
